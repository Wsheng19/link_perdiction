...
def run_model_DBLP(args):
    ...
    for test_edge_type in dl.links_test['data'].keys():
        ...
        net = myGAT(g, args.edge_feats, len(dl.links['count']) * 2 + 1, in_dims, args.hidden_dim, num_classes,
                    args.num_layers, heads, F.elu, args.dropout, args.dropout, args.slope, args.residual,
        ...
        early_stopping = EarlyStopping(patience=args.patience, verbose=True,
                                       save_path='E:/busy_studing/link_prediction/checkpoint/checkpoint_{}_{}.pt'.format(args.dataset, args.num_layers))
        ...
        for epoch in range(args.epoch):
            ...
            for step, start in enumerate(range(0, len(train_pos_head_full), args.batch_size)):
                ...
                # print training info
                print('Epoch {:05d}, Step{:05d} | Train_Loss: {:.4f} | Time: {:.4f}'.format(epoch, step,
                                                                                            train_loss.item(),
                                                                                            t_end - t_start))
                ...
                with torch.no_grad():
                    ...
                    labels = torch.FloatTensor(
                        np.concatenate([np.ones(valid_pos_head.shape[0]), np.zeros(valid_neg_head.shape[0])])).to(
                        device)
                    logits = net(features_list, e_feat, left, right, mid)
                    logp = F.sigmoid(logits)
